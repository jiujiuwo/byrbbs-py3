...
import scrapy
import re
from byrbbs.items import ByrbbsSectionItem
from byrbbs.spiders.byrbbs_config import HEADERS, LOGIN_FORMDATA
from datetime import datetime

class ByrbbsSectionSpider(scrapy.Spider):
        pipeline = ['ByrbbsSectionPipeline']
        name = "byrbbs_section"
        ...
        def logged_in(self, response):
            for url in self.start_urls:
                item = ByrbbsSectionItem()
                num = int(url[-1])
                item['top_section_num'] = num + 1  # 使存储的类别号从1开始
                item['top_section_name'] = self.top_section_name[num]
                yield scrapy.Request(url, meta={'cookiejar': response.meta['cookiejar'], 'item': item}, headers=HEADERS,
                                     callback=self.parse)
        ...
        def parse(self, response):
            ...
            for result in results:
                ...
                if rs:
                    next_url = 'http://bbs.byr.cn/section/ajax_list.json?uid=' + LOGIN_FORMDATA['id'] + '&root=sec-%s' % \
                                                                                                        rs[0]
                    ...
                    yield scrapy.Request(response.urljoin(url),
                                         meta={'cookiejar': response.meta['cookiejar'], 'item': item}, headers=HEADERS,
                                         callback=self.parse_article_total)
